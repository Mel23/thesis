...
\newcommand\ioon[2]{[#1$\;${\scshape{#2}}]}      % ion, i.e., [CII] = \\ioon{C}{ii}
...
\label{sec:Activity}
We use flux measurements from the 2012 release of the Oh2011 catalogue \citep{Oh2011} to classify disc galaxies as either star-forming, composite, AGN, LINER, or quiescent (also known as ``undetermined''). This method employs ratios of \ioon{O}{iii}/$\rm H\beta$ fluxes as a function of \ioon{N}{ii}, \ioon{S}{ii}, or \ioon{O}{i} over $\rm H\alpha$ according to the BPT diagnostics. Our method for selecting AGN is the same as used by \citet{Schawinski2007,Schawinski2010}. First, we use the \ioon{N}{ii}/H$\alpha$ ratio (Figure~\ref{BPT}a). Any galaxy that does not have $\rm S/N>3$ for any of the four lines is unclassifiable via this method (possibly due to being gas-poor) and labeled ``undetermined.'' Next, any galaxy which falls below the \citet{Kewley2001} extreme starburst line is classified as star-forming, and those that fall between this and the \citet{Kauffmann2003a} empirical starburst line are classified as composite. We note that some of these composite galaxies may be potential AGN, but we cannot cleanly separate the AGN contribution from star formation and thus exclude them from our sample \citep{Schawinski2010}. 

Next, we identify the remaining galaxies (above the extreme starburst line) as either Seyfert AGNs or LINERs. \citet{Kewley2006} showed that both \ioon{O}{i}/H$\alpha$ and  \ioon{S}{ii}/H$\alpha$ diagrams are better-suited to distinguish AGN from LINERs; we thus use diagram (c) in Figure~\ref{BPT} if these galaxies also have $\rm S/N>3$ in \ioon{O}{i}. For galaxies which do not have $\rm S/N>3$ in \ioon{O}{i}, but do in \ioon{S}{ii}, we use diagram (b). In both cases, we use the AGN-LINER division line of \citet{Kewley2006}. For the remaining galaxies, we use diagram (a) and implement the AGN-LINER division line of \citet{Schawinski2007}.
